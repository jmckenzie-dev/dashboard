...
so the dashboard never fetches them.
...
- `bwrap` is the only sandboxing mechanism wrapping opencode processes. No
  other unrecognized launchers exist. The `bwrap` args convention (using `--`
  to separate its own flags from the wrapped command) is stable and unlikely
...
### Step 3: Archive ALL 15 stuck sessions in SQLite

Run this SQL on the opencode database to immediately remove ALL stuck sessions
from the dashboard's view (the SQL query filters `WHERE time_archived IS NULL`).

```sql
UPDATE session SET time_archived = 1782493267000
WHERE id IN (
  -- Visible in dashboard (within LIMIT 200)
  ...
  -- Beyond LIMIT 200 but still stuck (good hygiene)
...
Archiving all 15 provides defense-in-depth: even if the code fix has a bug or
is delayed, these sessions won't reappear.
...
`ses_1f750b19dffeQfAVShN1uCS1wO`. Temporarily add in `opencode.ts`:
...
3. **Pre-restart: archive stuck sessions** — run the SQL from Step 3 against
   the opencode database at `~/.local/share/opencode/opencode.db`:
   ```bash
   sqlite3 ~/.local/share/opencode/opencode.db \
     "UPDATE session SET time_archived = $(date +%s)000 WHERE id IN (...)"
   ```

4. **Restart**: `./restart_dashboard.sh` — must restart the container cleanly.

5. **Dashboard inspection** (after restart + SQL archive):
...
6. **Process scan verification**: From inside the container, verify that
...
7. **API confirmation**: Verify `/session/status` returns `{}` (confirming no
...
- None resolved — the user confirmed both assumptions (archive all 15, bwrap
  is stable), and the plan is comprehensive.
