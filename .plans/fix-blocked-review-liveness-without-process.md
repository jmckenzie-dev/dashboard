...
### Option A: Require `hasProcessSessionId` for `blocked_review` `active_tool` liveness (Recommended)

In `directReason()` in `opencode-liveness.ts`, when `hasActiveTool` is true
and the candidate's status is `blocked_review`, additionally require
`hasProcessSessionId` to confirm the TUI process is alive. If the process is
gone, fall through to `cwd_allocated` or `hidden_stale`.

**Pros:**
- Targeted fix — only affects `blocked_review` sessions (the known gap)
- Uses existing signals (`status`, `hasProcessSessionId`) already in
  `OpenCodeLivenessCandidate`
- Doesn't change behavior for `working` sessions with active tools (where a
  flagless TUI legitimately has no `-s` in argv)
- Doesn't require adding new fields to liveness interfaces
- Easy to test

**Cons:**
- Slightly couples liveness logic to status inference semantics (knows that
  `blocked_review` implies `submit_plan`/`plan_exit`)
- If a future code path produces `blocked_review` without `submit_plan`, the
  check would need updating

### Option B: Add `activeToolName` field to `OpenCodeLivenessCandidate`

Add an optional `activeToolName` field to the candidate interface and check
specifically for `submit_plan`/`plan_exit` tool names instead of relying on
`blocked_review` status as a proxy.

**Pros:**
- More precise — checks the actual tool name, not a status proxy
- Decouples liveness from status inference semantics

**Cons:**
- Requires changing the `OpenCodeLivenessCandidate` interface and all callers
- More invasive change with more surface area for bugs
- The tool name is already captured indirectly via status (only
  `submit_plan`/`plan_exit` produce `blocked_review`)

### Option C: Shorten `ACTIVE_TOOL_LIVENESS_MAX_AGE_MS` for `blocked_review` sessions

Add a separate, shorter staleness bound for `blocked_review` sessions (e.g., 5
minutes) while keeping 30 minutes for other statuses.

**Pros:**
- Still uses time-based approach (familiar pattern)
- Doesn't require process-level confirmation

**Cons:**
- Still a race — a session whose TUI died 4 minutes ago would remain visible
  for 1 more minute
- Arbitrary timeout doesn't match the semantics (the issue is process death,
  not age)
- Could hide legitimate long reviews that happen to take >5 minutes
