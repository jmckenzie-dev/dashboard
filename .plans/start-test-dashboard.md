...
7. **Port selection** — `pick_port()`:
   - base = random in `[50001..59999]` via `$RANDOM` scaled, or `shuf -i 50001-59999 -n 1` if available (fallback to `$RANDOM` math).
   - probe with `ss -ltn` (and `ss -ltn6`): a port is free if neither `127.0.0.1:<p>` nor `0.0.0.0:<p>` nor `[::]:<p>` appears in `ss -Hltn sport = :<p>`.
   - on collision, increment (wrap at 60000 back to 50001); cap attempts at e.g. 200; exit 1 with diagnostic if none free.
...
9. **Start server**:
   ...
   - capture `SERVER_PID=$!`.
   - adapter-node binds `0.0.0.0` by default; we will additionally pass `HOST=127.0.0.1` (adapter-node honors `HOST`); document that if `HOST` is ignored, binding to 0.0.0.0 on a random high port is still acceptable for local testing.
10. **Wait for readiness**:
    - poll up to ~30s (e.g. 60 x 0.5s): use `ss -Hltn sport = :<port>` to confirm a listener exists, then a `curl -fsS -o /dev/null http://127.0.0.1:<port>/` (curl optional; if absent, fall back to `node -e` TCP connect probe). If auth is configured, expect non-401 on `/` only when auth disabled; treat any HTTP response (incl. 401) as "up".
...
Add a short "Test Dashboard" subsection under `## Development` in `README.md` describing usage, defaults, and the `--isolated` opt-in. Keep it tight (~10 lines).

### Step 4 — Add to TODO.md / worklog (repo convention)

Per AGENTS.md, track the work. Implementation agent should move the relevant TODO entry to DONE and write a `worklog/` summary.

## Validation Plan

After implementation, run from a clean checkout on a feature branch:

1. **Static**: `bash -n start_test_dashboard.sh` (syntax check, no execution).
...
3. **Real run**:
   ...
   - Ctrl-C the script; confirm the listener is gone (`ss` returns nothing) and the log mentions "Stopped".
...
8. **Existing production service untouched**: confirm `systemctl --user status ai-agent-dashboard.service` is not restarted and port 35001 is unaffected.
...
- **adapter-node ignores `HOST` env**: minor; binding 0.0.0.0 on a random high port is still local-only in practice. Mitigation: prefer printing `http://127.0.0.1:<port>` regardless; the URL works either way.
...
- **`$RANDOM` modulo bias / `shuf` absent**: low impact (just skews the starting port). Mitigation: accept either `shuf` or `$RANDOM`; correctness of freeness check does not depend on uniformity.
...
- **Long-running build blocks the user**: acceptable; build is the point of pre-merge verification. Mitigation: print progress lines (`Building...`) so the user sees activity.
