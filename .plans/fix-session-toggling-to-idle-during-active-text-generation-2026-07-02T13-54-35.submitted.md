---
submitted-at: "2026-07-02T13:54:35.571Z"
...
While the target session is actively generating, capture its inference inputs:
- Preferred (local): `npm run dump:sessions -- --session "Plannotator" --json
  --no-parts`, repeated 3–4 times across ~20s. Inspect per tick:
  `sessionStatus`, `lastActivityMs`, `latestTool.active`, `livenessReason`,
  `parts` (note whether new `text`/`reasoning` parts appear with advancing time).
  - Note: the standalone `dump:sessions` compile currently fails (`tsc` invoked
    without project flags — `esModuleInterop`/`target` errors unrelated to this
    change). If it blocks, fix the dump script's tsconfig invocation, OR use the
    authenticated `GET /api/status/diagnose` endpoint
    (`src/routes/api/status/diagnose/+server.ts`) against the running service.
- Decision matrix:
  - `sessionStatus==='busy'` but status still flips idle ⇒ pure lastActivity bug
    (Step 2 fixes it).
  - `sessionStatus` null/absent during generation while parts advance ⇒ also
    scoping/enrichment-staleness (Step 2 + Step 3 needed).
  - `lastPartTime` advancing while `lastActivityMs` large ⇒ confirms session-row
    vs part-time mismatch (Step 2).
...
- `./restart_dashboard.sh` after deploying the change.
- `bash run_tests.sh` — all self-tests green (status inference, liveness,
  optimize-poller, visibility hysteresis, process poller).
- Reproduce: with the target session actively generating, poll
  `GET /api/status/diagnose` (or `dump:sessions`) every few seconds for ~30s.
...
- Does the session row `time.updated` advance during streaming, or only on
  discrete `session.updated` events? Determines whether Step 3 is strictly
  necessary or just defensive.
