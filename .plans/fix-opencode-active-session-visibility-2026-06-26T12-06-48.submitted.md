---
submitted-at: "2026-06-26T12:06:48.038Z"
...
3. Tighten OpenCode visibility filtering.
   ...
   - For OpenCode sessions, keep only:
     ...
     - very recent activity fallback for cwd-inaccessible active sessions.
   - Do not keep old OpenCode `idle`, `complete`, or `error` sessions purely via broad recency/error windows.
...
- Runtime checks after `./restart_dashboard.sh`:
  - `GET /api/status/diagnose` no longer lists the 15-17h stale `error` sessions unless they have a direct active signal.
  - `GET /api/status/diagnose` no longer lists the 6/24 stale `idle` sessions solely because another process shares their directory.
...
- Over-tight filtering could hide an open but completely idle session with no direct signal and no readable cwd. Mitigation: make visibility reasons diagnostic and tune the recent fallback only after observing live behavior.
...
## Open Questions
- Should a just-completed OpenCode session remain visible briefly as `complete`, or should it disappear immediately unless its instance is still cwd/direct-live?
- If multiple idle sessions are intentionally open in the same directory, is “newest N by process count” acceptable, or should we require an upstream/OpenCode change to expose active session ids?